  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "20px", alignItems: "start" }}>
      ...
      <div style={{ background: "white", borderRadius: "20px", border: "1px solid #FFD4C2", overflow: "hidden", boxShadow: "0 2px 12px rgba(255,160,122,0.1)" }}>
        {/* Header */}
        <div style={{ padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #FFE8DC" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", padding: "12px 16px 0" }}>
          {DAYS.map(d => (
            <div key={d} style={{ textAlign: "center", fontSize: "11px", fontWeight: 700, color: "#C4826A", padding: "8px 0", fontFamily: "'JetBrains Mono'" }}>{d}</div>
          ))}
        </div>
          ...
          {calendarDays.map((d, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: "10px", cursor: d.currentMonth ? "pointer" : "default",
            ...
            >
              <span style={{ fontSize: "13px", fontWeight: d.isToday ? 700 : d.currentMonth ? 500 : 400, color: d.isToday ? "white" : d.currentMonth ? "#3D1A0A" : "#E8C4B8" }}>
                {d.day}
              </span>
              {d.hasEvent && d.currentMonth && (
                <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: d.isToday ? "white" : "#FFA07A", marginTop: "2px" }} />
              )}
            </div>
          ))}
        </div>
        ...
        {/* Legend */}
        <div style={{ padding: "12px 24px", borderTop: "1px solid #FFE8DC", display: "flex", alignItems: "center", gap: "16px" }}>
      ...
      <div>
        {selectedDate ? (
          <div>
            <h3 style={{ fontFamily: "'Syne'", fontSize: "16px", fontWeight: 800, color: "#3D1A0A", marginBottom: "14px" }}>
              {MONTHS[currentMonth]} {selectedDate.day}, {currentYear}
            </h3>
            {selectedEvents.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                {selectedEvents.map(event => (
                  <div key={event.id} style={{ background: "white", borderRadius: "14px", border: "1px solid #FFD4C2", padding: "14px", boxShadow: "0 2px 8px rgba(255,160,122,0.08)" }}>
                    ...
                    {event.reminderEmail && (
                      <div style={{ marginTop: "8px", padding: "6px 10px", borderRadius: "8px", background: "#FFF5F0", border: "1px solid #FFD4C2" }}>
                        <p style={{ fontSize: "10px", color: "#FFA07A", fontWeight: 600 }}>🔔 Reminder → {event.reminderEmail}</p>
          ...
          <div style={{ textAlign: "center", padding: "40px 16px", background: "white", borderRadius: "14px", border: "1px solid #FFD4C2" }}>
            <div style={{ fontSize: "32px", marginBottom: "10px" }}>📅</div>
            <p style={{ fontSize: "13px", fontWeight: 600, color: "#3D1A0A", marginBottom: "6px" }}>Select a date</p>
            <p style={{ fontSize: "12px", color: "#C4826A" }}>Click any date to see events</p>
          </div>
        )}
